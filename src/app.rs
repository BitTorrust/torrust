...
use clap::Parser;   
use crate::cli::Args;
use std::{fs, convert::TryInto, path::PathBuf};
...
pub fn build_tracker_request(torrent: &Torrent) -> TrackerRequest {
    ...
    let tracker_request = TrackerRequest::new(info_hash, PEER_ID, 6882, 0, 0, left_to_download as usize, true, Some(event));
    
    return tracker_request;
}

pub fn send_request(tracker_request: TrackerRequest, tracker: Tracker) -> Result<TrackerResponse, Error> {
    let tracker_host = tracker.get_tracker_host();
    let tracker_port = tracker.get_tracker_port();
    let url = tracker_request.into_url(tracker_host, tracker_port).unwrap();
