#[cfg(test)]
pub mod unittest {
    use std::{collections::HashMap, net::{SocketAddr, Ipv4Addr, IpAddr}};

    use bit_vec::BitVec;

    use crate::{pieces_selection::{simple_selection::SimpleSelector, PiecesSelection}, http::Peer};

    #[test]
    pub fn select_pieces_with_one_peer_having_all_pieces() {
        let mut peers_bitfields : HashMap<Peer, BitVec> = HashMap::new();
        let seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 6999));
        ...
        let selection : HashMap<u32, Option<Peer>> = SimpleSelector::pieces_selection(mybitfield, peers_bitfields);
    ...
    pub fn select_pieces_with_two_peers_having_half_pieces_each() {
        let mut peers_bitfields : HashMap<Peer, BitVec> = HashMap::new();
        let bitfield_length = 9;
        let first_part_seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 6999));
        ...
        let second_part_seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 6998));
    ...
    pub fn select_pieces_with_two_peers_having_overlapping_pieces() {
        ...
        let first_part_seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 0001));
        let mut first_part_seeder_bitfield = BitVec::from_elem(bitfield_length, true);
        first_part_seeder_bitfield.set(3, false);

        let second_part_seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 0002));
        let mut second_part_seeder_bitfield = BitVec::from_elem(bitfield_length, true);
        second_part_seeder_bitfield.set(0, false);
        second_part_seeder_bitfield.set(1, false);
    ...
    pub fn select_pieces_with_one_peer_having_one_missing_that_we_have() {
        ...
        let seeder : Peer = Peer::from_socket_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 0002));
